...
1️⃣ User Triggers Onboarding (Backstage)
The user opens the Backstage UI and selects the “Onboard User to Org” tile.
Inputs Provided:
...
Step B (Publish): Creates the repository quantum-lab-x/<repoName> with repoVisibility: public (to allow secret access on Free Tier).

3️⃣ CODEOWNERS Applied Automatically
The template automatically generates the .github/CODEOWNERS file during the push:

Plaintext
* @quantum-lab-x/{{ values.teamName }}
4️⃣ Trigger GitHub Actions Automation
Backstage uses the github:actions:dispatch action to trigger the create-team.yml workflow located in the quantum-lab-x/platform-automation repository.
